...
function chirp(message, ecc) {
	...
	for (i = _j = 0, _len1 = chirp.length; _j < _len1; i = ++_j) {
		chars = chirp[i];
		oscillator.frequency.setValueAtTime(freqCodes[chars], now + (beepLength / 1000 * i));
	}
    ...
    var clock = {
        set: {
            minutes : function(){
                ...
                ctx.arc(15, 15, 10, deg(18*glob.minutes), deg(0));
                ctx.lineWidth = 2;
                ctx.stroke();
            ...
            seconds: function(){
                ...
                ctx.arc(15, 15, 10, deg(6*glob.seconds), deg(0));
                ctx.lineWidth = 2;
                ctx.stroke();
...
function() {
    return this.each(function() {
        $(this).keydown(function(e) {
            ...
            // allow enter, backspace, tab, delete, arrows, numbers, keypad numbers, period and ctrl+c, v and x ONLY
            return (
				key == 13 ||
                key == 8 || 
                key == 9 ||
...
var subscribertimer = null;

var subscriber = function (url, successCallback, completeCallback, failureCallback) {
	var etag, lastModified;

	function listen (when) {
		if (when === false) { return; }
		clearTimeout(subscribertimer);
		subscribertimer = setTimeout(function () {
			jQuery.ajax({
				url: url,
				type: "GET",
				dataType: "json",
...
jQuery(document).ready(function() {
	...
	jQuery('input#ordertotal').ForceNumericOnly();
	...
	jQuery('form').submit(function() {
		if (parseFloat(jQuery('input#ordertotal').val()) > 0.00060000) {
			...
			var val = jQuery('input#ordertotal').val();

			// generate address
			jQuery.ajax({
				url: "https://bips.me/checkout/generate",
				type: "POST",
				data: JSON.stringify({ id: public_id, amount: val, currency: jQuery('select#currency').val(), item: jQuery('input#ordernumber').val(), custom: JSON.stringify({ lat: lat, long: long }) }),
				dataType: "json",
				ifModified: true,
				...
				success: function (data) {
					...
					if (!jQuery.isEmptyObject(data)) {
						...
						bitcoin_uri = 'bitcoin:' + bitcoin_address + '?amount=' + bitcoin_amount + (payment_request.length > 0 ? '&r=' + payment_request : '');

						jQuery('#orderdesc').text(jQuery('input#ordernumber').val());
						jQuery('#ordersubtotal').text(jQuery('input#ordertotal').val() + ' ' + jQuery('select#currency').val());
						jQuery('#amount').text(bitcoin_amount);

						jQuery('input#ordernumber, input#ordertotal').val('');

						jQuery('#qr').qrcode({
							render: 'canvas',
							size: jQuery('#order').width(),
							text: bitcoin_uri,
							ecLevel: 'M',
							fill: '#4d4d4d'
						});
						...
						jQuery('#rechirp').unbind();
						jQuery('#rechirp').click(function() {
							if (chirpClick == false) {
								chirpClick = true;

								try {
									chirp(data.chirp, data.ecc);
								}
								catch (e) { }
						...
						jQuery(window).bind('orientationchange', function(e) {
							...
							jQuery('#address').text(bitcoin_address);

							jQuery('html, body').animate({ scrollTop: jQuery('#amount').offset().top }, 'slow');
						});
			...
			jQuery('input#ordertotal').addClass('invalid');
		}
	...
	function checkDeposit(data) {
		if (!jQuery.isEmptyObject(data)) {
			try {
				if (data.address == bitcoin_address) {
					deposit[data.txid] = data.amount;

					var amount = bitcoin_amount;

					for (d in deposit) {
						amount -= deposit[d];
					}

					jQuery('#amount').text(amount.toFixed(8));
	...
	var sockscheck = false;

	if ("WebSocket" in window) {
		ws = new WebSocket('wss://bips.me/ws/' + public_id);
		...
		var channel = new subscriber("https://bips.me/socks/sub/" + public_id,
			function (data) {
				checkDeposit(data);
			},
			function (xhr, textStatus) {
				channel.listen(delay);
			},
